 ...
 * Created on February 27th, 2022, 1:00 pm
...
TriMatx *fillMat(int rows){
    ...
    triMt->size=rows;
    triMt->col=fillAry(rows);
    triMt->indx=fillIdx(rows);
    triMt->data=fillAry(triMt->col,rows);
    return triMt;
}
    ...
    for(int pos=0;pos<triMt->size-1;pos++){
        for(int lst=pos+1;lst<triMt->size;lst++){
            if(triMt->col[triMt->indx[pos]]>triMt->col[triMt->indx[lst]]){
                triMt->col[triMt->indx[pos]]=triMt->col[triMt->indx[pos]]^triMt->col[triMt->indx[lst]];
                triMt->col[triMt->indx[lst]]=triMt->col[triMt->indx[pos]]^triMt->col[triMt->indx[lst]];
                triMt->col[triMt->indx[pos]]=triMt->col[triMt->indx[pos]]^triMt->col[triMt->indx[lst]];
                int * temp = triMt->data[triMt->indx[pos]];
                triMt->data[triMt->indx[pos]]=triMt->data[triMt->indx[lst]];
                triMt->data[triMt->indx[lst]]=temp;
